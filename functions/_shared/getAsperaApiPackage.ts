...
const getAsperaApiPackage: SLF.GetAsperaApiPackage = async (token: SLF.AsperaApiToken, config: SLF.AsperaApiPackageConfig): Promise<SLF.AsperaApiPackageInfo | null> => {
    ...
    try {
        ...
        if (config.contentsFileId !== undefined) {
            ...
            if (packageInfo.contentsFileId !== config.contentsFileId) {

                throw new Error("getAsperaApiPackage() contentsFileId did not match package { .contents_file_id }");
            }

            if (packageInfo.fileId !== `${parseInt(config.contentsFileId) - 1}`) {

                throw new Error("getAsperaApiPackage() fileId did not match package { .file_id }");
            }
        }
